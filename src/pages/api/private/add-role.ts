...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (roles[roleName]) {
      res.status(400).json({ message: "Такая роль уже есть", success: false });
      return;
    }

    // Добавляем новую роль с пустым списком прав
    roles[roleName] = [];

    fs.writeFileSync(filePath, JSON.stringify(roles, null, 2), "utf-8");

    res.status(200).json({ message: "Роль успешно добавленна", success: true, updated: roles });
  } catch (err) {
    console.error(err);
